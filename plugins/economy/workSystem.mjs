 ...
 *
 * Balancing target: a player can complete at most six shifts per hour.
 * Job payouts are deliberately modest because the bot already has daily,
 * crime, gambling, and shop economies.
 */
...
// Compatibility for players who already chose a job in the previous system.
...
export function clampEnergy(value) {
  return Math.max(0, Math.min(MAX_ENERGY, Number.isFinite(Number(value)) ? Number(value) : MAX_ENERGY));
}
