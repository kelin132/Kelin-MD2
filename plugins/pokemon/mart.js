...
// 8 pages of items, .mart page <n> to navigate
// 30-second per-user cooldown on purchases

import { getTrainer, addItem, hasItem } from "../../lib/pokemon/players.mjs";
import { getMartPage, getItemByIndex, TOTAL_PAGES, PAGE_LABELS } from "../../lib/pokemon/martItems.mjs";
...
export default {
  ...
  category: "pokemon",
  usage: ".mart [buy <number> [qty]] [page <n>]",
  ...
  async run({ sock, msg, sender, args }) {
    ...
    if (sub === "page") {
      ...
      const content  = await getMartPage(pageNum);
...
🛒 Buy:   *.mart buy <number> [qty]*`,
    ...
    if (sub === "buy") {
      ...
      if (!Number.isInteger(itemNumber) || itemNumber < 1) {
        return sock.sendMessage(jid, {
          text: "Usage: *.mart buy <number> [qty]*\nExample: `.mart buy 1 5`\n\nUse *.mart* or *.mart page <n> to browse numbered items.",
        }, { quoted: msg });
      }

      const itemData = getItemByIndex(itemNumber);
      if (!itemData) {
        return sock.sendMessage(jid, {
          text: `❌ Item number *${itemNumber}* not found.\nUse *.mart* to browse all numbered items.`,
        }, { quoted: msg });
      }
      const itemKey = itemData.id;
...
${itemData.desc}${itemData.category === "key"
  ? itemKey === "keystone"
    ? "\n\n💎 Use *.equip <pokémon>* to attach it!"
    : `\n\nUse *.item use ${itemKey}*`
  : ""}`,
      }, { quoted: msg });
    }
