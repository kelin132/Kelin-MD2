export default {
  ...
  async run({ sock, msg, text }) {
    ...
    try {
      // Lazy-load ytdl (optional dep — installed via npm install on the panel)
      let ytdl;
      try {
        ytdl = (await import("@distube/ytdl-core")).default;
      } catch {
        return sock.sendMessage(jid, { text: "❌ ytdl not installed on this server.\nRun: npm install @distube/ytdl-core" }, { quoted: msg });
      }
      ...
      if (/youtu\.?be/.test(text)) {
        ...
        // Search YouTube
        const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(text)}&hl=en`;
