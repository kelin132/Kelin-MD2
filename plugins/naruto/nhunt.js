 ...
 *   .nhunt jutsu        — view your jutsu list (in active battle)
 *   .nhunt jutsu <n>    — use the nth jutsu (in active battle)
 *   .nhunt item         — view your item bag (in active battle)
 *   .nhunt item <id>    — use a consumable/battle item (only in battle)
 *   .nhunt flee         — escape (5% ryo penalty + 10% max HP loss)
 */

import players, { getRankForLevel } from "../../lib/naruto/players.js";
...
/** Full jutsu list shown when player types .nhunt jutsu (no number). */
function buildJutsuMenu(p) {
  const lines = [`🌀 *Jutsu List*`, ``];
  if (!p.jutsu.length) {
  ...
  lines.push(``, `💙 Chakra: ${p.chakra}/${p.maxChakra}`);
  return lines.join("\n");
}

/** Full item list shown when player types .nhunt item (no ID). */
function buildItemMenu(p) {
...
/** Build the compact action menu shown to the player between turns. */
function buildMenu(hunt) {
  ...
  const jutsuCount = p.jutsu.length;
  const usableItems = p.inventory.filter(inv => {
    const def = itemsLib.find(x => x.id === inv.id);
    return def && ["consumable", "battle", "boost"].includes(def.type);
  });
  ...
  lines.push(
    ``,
    `*Choose your action:*`,
    `🥊 *.nhunt attack* — Basic attack`,
    `🌀 *.nhunt jutsu* — View Jutsu (${jutsuCount} move${jutsuCount !== 1 ? "s" : ""})`,
    `🎒 *.nhunt item* — View Items (${usableItems.length} usable)`,
    ``,
    `🏃 *.nhunt flee* — Escape (5% ryo + 10% HP penalty)`,
    `⏳ _3 minutes to act or battle auto-cancels._`,
  );
...
function applyLevelUps(player) {
  // Guard: ensure xpNeeded is valid to prevent permanent level-lock
  ...
  // Sync rank to the new level (was missing — players stayed at wrong rank after hunt level-ups)
  player.rank = getRankForLevel(player.level);
  ...
  async run({ sock, msg, sender, args }) {
    ...
    try {
      ...
      // ── JUTSU ────────────────────────────────────────────────────────────
        ...
        // No number provided → show the full jutsu list
        if (!args?.[1]) {
          return sock.sendMessage(jid, { text: buildJutsuMenu(p) }, { quoted: msg });
        }

        const idx = parseInt(args[1], 10) - 1;
        if (isNaN(idx) || idx < 0 || idx >= p.jutsu.length) {
          return sock.sendMessage(jid, {
            text: `❓ Invalid jutsu number. You have ${p.jutsu.length} jutsu.\n\nUse *.nhunt jutsu* to see your list.`
          }, { quoted: msg });
        }
      ...
      // ── ITEM ─────────────────────────────────────────────────────────────
        ...
        // No ID provided → show the full item list
        if (!args?.[1]) {
          return sock.sendMessage(jid, { text: buildItemMenu(p) }, { quoted: msg });
        }

        const itemId = args[1];
        const invIdx = p.inventory.findIndex(i => i.id === itemId);
        if (invIdx === -1) {
          return sock.sendMessage(jid, { text: `❌ You don't have \`${itemId}\` in your bag.\n\nUse *.nhunt item* to see your bag.` }, { quoted: msg });
        }
        ...
        return sock.sendMessage(jid, {
          text: `❓ Unknown action.\n\n*.nhunt attack* / *.nhunt jutsu* / *.nhunt item* / *.nhunt flee*`
        }, { quoted: msg });
      }
