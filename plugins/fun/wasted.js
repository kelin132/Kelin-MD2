...
export default {
  ...
  async run({ sock, msg }) {
    ...
    try {
      ...
      if (!uploadRes.ok) throw new Error(`Upload failed with HTTP ${uploadRes.status}`);
      ...
      const imgRes = await fetch(apiUrl, { signal: AbortSignal.timeout(15_000) });
      if (!imgRes.ok) throw new Error(`Wasted image API failed with HTTP ${imgRes.status}`);
      const result = Buffer.from(await imgRes.arrayBuffer());
