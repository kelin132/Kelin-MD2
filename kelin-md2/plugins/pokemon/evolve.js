...
💎 *Mega Evolution:* Buy a Mega Stone at *.mart page 8*, equip the Key Stone first (*.equip*), then use the Mega Stone to trigger Mega Evolution!

Buy regular evolution stones at *.mart page 4*`,
      }, { quoted: msg });
    }
    ...
    if (isMegaStone && trainer.keystoneEquippedTo !== pokeId) {
      return sock.sendMessage(jid, {
        text:
          `❌ *${found.displayName || found.name}* needs an equipped *💎 Key Stone* to Mega Evolve.\n` +
          `Use *.equip ${found.displayName || found.name}*, then try again.`,
      }, { quoted: msg });
    }
