...
/** Generate a unique 5-digit numeric pet ID for this owner. */
async function generatePetId(c, owner) {
  let id;
  do {
    id = String(Math.floor(10000 + Math.random() * 90000));
  } while (await c.findOne({ owner, petId: id }, { projection: { petId: 1 } }));
  return id;
}
...
export async function createPet(owner, speciesKey, imageUrl, makeActive = true) {
  ...
  const petId   = await generatePetId(c, owner);
