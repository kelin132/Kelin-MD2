  ...
  for (const cat of categories) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // Prefer an explicit category from the plugin metadata. Most plugins
        // live in a matching directory, but some group commands are kept in
        // admin for historical reasons and should still appear under Group.
        plugins.push({ ...plugin, category: plugin.category || cat });
...
export async function routeMessage(sock, msg, prefix = ".", ownerNumber = "", fromMe = false) {
  ...
  // Full JID from participant (groups) or remoteJid (DM). Resolve privacy
