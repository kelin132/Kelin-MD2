 ...
 *   generateTransferImage({ direction, amount, cash, bank })
 */
...
// ─── 6. BANK TRANSFER ──────────────────────────────────────────────────────────
export async function generateTransferImage({ direction, amount, cash, bank }) {
  const isDeposit = String(direction).toLowerCase() === "deposit";
  const accent = isDeposit ? "#45D483" : "#FF9F43";
  const title = isDeposit ? "DEPOSIT SUCCESSFUL" : "WITHDRAWAL SUCCESSFUL";
  const verb = isDeposit ? "Moved into bank" : "Moved to wallet";
  const width = 900;
  const height = 360;
  ...
  background.addColorStop(0, "#17161d");
  background.addColorStop(1, "#2a2730");
  ...
  ctx.beginPath();
  ctx.arc(740, 120, 180, 0, Math.PI * 2);
  ctx.fillStyle = accent;
  ctx.fill();
  ctx.globalAlpha = 1;
  ...
  ctx.fillText(isDeposit ? "🏦  DEPOSIT" : "🏦  WITHDRAWAL", 58, 64);
  ctx.font = "bold 24px Arial";
  ctx.fillStyle = accent;
  ctx.fillText(title, 58, 108);

  ctx.font = "bold 58px Arial";
  ctx.fillStyle = "#ffffff";
  ctx.fillText(`$${Number(amount || 0).toLocaleString()}`, 58, 192);
  ctx.font = "22px Arial";
  ctx.fillStyle = "#aaa5b2";
  ctx.fillText(verb, 62, 230);

  ctx.font = "bold 24px Arial";
  ctx.fillStyle = "#f6f3f8";
  ctx.fillText(`💰 Wallet   $${Number(cash || 0).toLocaleString()}`, 58, 300);
  ctx.fillText(`🏦 Bank     $${Number(bank || 0).toLocaleString()}`, 470, 300);
  ctx.font = "20px Arial";
  ctx.fillStyle = "#aaa5b2";
  ctx.fillText("AIDORU • AKIRA ECONOMY", 470, 334);
