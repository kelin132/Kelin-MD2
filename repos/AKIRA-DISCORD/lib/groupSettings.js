...
function storageKey(key) {
  ...
  const match = normalized.match(/^discord:([^@]+)@g\.us$/);
  return match ? `discord:${match[1]}` : normalized;
}
...
class GroupSettings {
  get(jid) {
    return cache[storageKey(jid)] || {};
  }

  set(jid, settings) {
    const key = storageKey(jid);
    cache[key] = { ...cache[key], ...settings };
    mongoSet(key, cache[key]);
  }
