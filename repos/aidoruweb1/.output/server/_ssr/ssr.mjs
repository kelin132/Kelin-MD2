...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-B39CcLPg.mjs").then((n) => n.t).then((m) => m.default ?? m);
	...
	try {
		...
		if (normalized.status === 404 && request.method === "GET") {
			if ((request.headers.get("accept") ?? "").includes("text/html")) {
				...
				else console.error("SPA fallback: no built index.html found in .output/public, dist/, or public/");
			}
		}
		return normalized;
	} catch (error) {
