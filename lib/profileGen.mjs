...
export async function generateProfileImage(
  ...
  const drawInfo = (y, icon, label, value) => {
    ctx.textAlign = "left";
  ...
  ctx.font = "bold 24px Sans";
  ctx.fillText("── ✦ 𝗦𝗧𝗔𝗧𝗦 ✦ ──", W / 2, 675);
  drawInfo(725, "🌟", "Active", data.daysActive ?? data.reach ?? 0);
  drawInfo(765, "🃏", "Cards", data.cards ?? 0);
  drawInfo(805, "🎮", "Games", data.games ?? 0);
  drawInfo(845, "🐾", "Pokémon", data.pokemon ?? 0);

  ctx.textAlign = "center";
  ctx.fillStyle = "#f4b8d2";
  ctx.font = "bold 24px Sans";
  ctx.fillText("── ✦ 𝗪𝗘𝗔𝗟𝗧𝗛 ✦ ──", W / 2, 915);
  drawInfo(965, "💰", "Wallet", `$${money(data.wallet)}`);
  drawInfo(1005, "🏦", "Bank", `$${money(data.bank)}`);
  drawInfo(1045, "💎", "Diamonds", money(data.diamonds));
  drawInfo(1085, "🎒", "Items", data.items ?? 0);

  ctx.textAlign = "left";
  ctx.font = "bold 21px Sans";
  ctx.fillStyle = "#f4f0f2";
  ctx.fillText(`📝 ${fitText(ctx, data.bio || "None", 690)}`, 98, 1145);
  ctx.fillText(`🏰 Guild: ${fitText(ctx, data.guild || "None", 610)}`, 98, 1182);
  ctx.fillStyle = "#dfb8cc";
  ctx.font = "20px Sans";
  ctx.fillText(`📅 Joined: ${fitText(ctx, data.joined || "Unknown", 600)}`, 98, 1219);
