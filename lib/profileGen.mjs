...
async function loadProfileFrame(frameId) {
  const normalized = String(frameId || "").trim().match(/^frame-(0[1-9]|1[0-9]|2[0-9]|3[0-6])$/)?.[0];
...
export async function generateProfileImage(
  ...
  if (profileFrame) {
    const frameSize = (avatarRadius + 27) * 2;
    drawImageCover(
      ctx,
      profileFrame,
      avatarX - frameSize / 2,
  ...
  const cardCount = Math.max(0, Number(data.cards) || 0);
  const pokemonCount = Math.max(0, Number(data.pokemon) || 0);
  ctx.fillStyle = "#d8b9f2";
  ctx.font = "bold 25px Sans";
  ctx.fillText(`Cards ${cardCount.toLocaleString()}   •   Pokémon ${pokemonCount.toLocaleString()}`, W / 2, barY + 218);

  ctx.fillStyle = "#ead8ff";
  ctx.font = "bold italic 29px Georgia";
  ctx.fillText(`“${fitText(ctx, data.bio || "No bio set.", 880)}”`, W / 2, barY + 258);
