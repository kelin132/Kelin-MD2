...
export async function generateProfileImage(
  ...
  /*
   * Soft shadow behind the circular avatar frame.
   */
  ctx.save();
  ...
  /* Circular frame only; no square portrait frame or corner ornaments. */
  ctx.save();
  ctx.beginPath();
  ctx.arc(avatarX, avatarY, avatarRadius + 22, 0, Math.PI * 2);
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(avatarX, avatarY, avatarRadius + 31, 0, Math.PI * 2);
  ...
  ctx.shadowColor = "rgba(255, 235, 245, 0.78)";
  ctx.shadowBlur = 8;
  ctx.shadowOffsetY = 2;
  ...
  ctx.fillText(
    fitText(ctx, `${roleLabel}  •  ${data.levelRole?.emoji || "~"} ${data.levelRole?.name || "Newcomer"}`, 880),
    W / 2,
    usernameY + 54,
  );

  const barX = 100;
  const barY = usernameY + 100;
  const barW = W - 200;
  const barH = 16;
  ...
  ctx.font = "bold italic 34px Sans";
  ctx.fillText(`♜ ${fitText(ctx, guildName, 880)} ♜`, W / 2, barY + 135);
  ...
  ctx.fillStyle = "#351b34";
  ctx.font = "bold italic 27px Sans";
