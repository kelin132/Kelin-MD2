...
const DATA_DIR = path.resolve(process.env.RPG_DATA_DIR || "database");
const CHAR_PATH = path.join(DATA_DIR, "dungeonCharacters.json");
const RUN_PATH = path.join(DATA_DIR, "dungeonRuns.json");
...
function readJson(filePath, fallback) {
  ...
  try {
    ...
    log("warn", `[rpg] Could not read ${filePath}: ${error.message}`);
    return fallback;
  }
}
...
export const runs = new Map(
  Object.entries(readJson(RUN_PATH, {})).map(([jid, run]) => [jid, run]),
); // jid -> active run state
...
function loadChars() {
  return readJson(CHAR_PATH, {});
}
function saveChars(all) {
  writeJsonAtomic(CHAR_PATH, all);
}
function saveRuns() {
  writeJsonAtomic(RUN_PATH, Object.fromEntries(runs));
}
...
export const RPG_SHOP_ITEMS = [
  ...
  },
  {
    ...
    name: "Healing Potion",
    price: 20,
    description: "Restores up to 40% max HP",
  },
];

export function buyRpgShopItem(jid, itemQuery) {
  const character = getCharacter(jid);
  if (!character) return { ok: false, reason: "no-character" };

  const query = String(itemQuery || "").trim().toLowerCase();
  const numericIndex = /^\d+$/.test(query) ? Number(query) - 1 : -1;
  const item =
    RPG_SHOP_ITEMS[numericIndex] ||
    RPG_SHOP_ITEMS.find(
      (candidate) =>
        candidate.id === query ||
  ...
  if (gold < item.price) return { ok: false, reason: "too-poor", item };

  character.gold = gold - item.price;
  if (item.type === "potion") {
    character.potions = (character.potions || 0) + 1;
  } else {
    character.inventory = Array.isArray(character.inventory) ? character.inventory : [];
    character.inventory.push({
      id: `${item.slot}-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
      name: item.name,
      slot: item.slot,
      bonus: item.bonus,
      rarity: item.rarity,
  ...
  saveCharacter(jid, character);
  return { ok: true, item, gold: character.gold, potions: character.potions };
}
