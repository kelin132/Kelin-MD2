...
export async function downloadInstagramVideo(url) {
  ...
  const tempFilePath = join(tmpdir(), `ig_video_${Date.now()}.source.mp4`);
  const compatibleFilePath = join(tmpdir(), `ig_video_${Date.now()}.mp4`);
  ...
  try {
    ...
    await execFileAsync(
      ...
      [
        ...
        // Prefer WhatsApp-friendly streams, but keep fallbacks for older posts.
        '-f', 'bestvideo[vcodec^=avc1][ext=mp4]+bestaudio[acodec^=mp4a]/best[ext=mp4][vcodec^=avc1]/best[ext=mp4]/best',
    ...
    // A file named .mp4 is not necessarily WhatsApp-compatible: yt-dlp can
    // fall back to VP9/AV1 video or an unsupported audio codec. Normalize the
    // stream and move the MP4 metadata to the front for reliable playback.
    await execFileAsync(
      FFMPEG_BIN,
      [
    ...
    if (!existsSync(compatibleFilePath)) {
      throw new Error('Compatible MP4 was not produced.');
    }

    return {
      filePath: compatibleFilePath,
      cleanup:  () => {
        for (const file of [tempFilePath, compatibleFilePath]) {
