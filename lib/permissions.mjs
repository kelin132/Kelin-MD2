...
import { userJidFilter } from "./whatsappIdentity.mjs";
    ...
    if (existsSync(MODS_FILE)) {
      ...
      return list.map((entry) => {
        const value = typeof entry === "string"
          ...
          : { ...entry };
        const num = String(value.num || "")
          .split("@")[0]
          .split(":")[0]
          .replace(/\D/g, "");
        return { ...value, num };
      }).filter((entry) => entry.num);
    }
  } catch { /* ignore */ }
  return [];
}
...
export function getMods() {
  return getModsData()
    .map(e => String(e.num || "").split("@")[0].split(":")[0].replace(/\D/g, ""))
    .filter(Boolean);
}
...
export async function getPermissions(sender, ownerNumber = "", { fromMe = false, sock, chatId } = {}) {
  ...
  try {
    ...
    if (isJailed && user?.jailUntil && user.jailUntil <= Date.now()) {
      isJailed = false;
      db.collection("users")
        .updateOne(userJidFilter(sender), { $set: { jailed: false, jailUntil: null } })
        .catch(() => {});
    }
