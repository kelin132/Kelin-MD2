...
export async function getPermissions(sender, ownerNumber = "", { fromMe = false, sock, chatId } = {}) {
  ...
  if (fromMe) {
    ...
    if (!ownerDigits) return _ownerPerms();                      // not configured — legacy mode
    const botNum = jidToNum(sock?.user?.id || "");
    if (!botNum || botNum === ownerDigits) return _ownerPerms(); // bot IS the owner number
    // Otherwise fromMe alone is not enough — fall through to number comparison
  }
