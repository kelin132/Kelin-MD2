...
async function loadCardLeaderboard(seriesQuery = "") {
  ...
  if (seriesLabel) {
    const pipeline = [
      // `cards.series` is indexed; matching it before filtering the array
      // avoids scanning every card collection for a series leaderboard.
      { $match: { "cards.series": seriesLabel } },
    ];
    pipeline.push({
      $project: {
        userId: 1,
