...
export function formatCardLeaderboard({ rows, seriesLabel = "" }) {
  const title = seriesLabel ? `🌸 𝐒𝐄𝐑𝐈𝐄𝐒 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐎𝐑𝐒 · ${seriesLabel}` : "🌸 𝐂𝐀𝐑𝐃 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐎𝐑𝐒";
  const footer = seriesLabel ? `✦ 𝐒𝐄𝐑𝐈𝐄𝐒 · ${seriesLabel.toUpperCase()} ✦` : "✦ 𝐀𝐍𝐈𝐌𝐄 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐈𝐎𝐍 ✦";
  const lines = rows.map((row, index) => {
    const badge = RANK_BADGES[index] || `${index + 1}️⃣`;
    return `│ ${badge} ${row.name}  ── 🃏 ${row.total.toLocaleString()}`;
  });
