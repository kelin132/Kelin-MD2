...
const MEDIA_TIMEOUT_MS   = 2_500;   // card media should never block a command for long
 ...
 * "Series Name" layer. Reading it is more reliable than guessing a series
 * from a character name and works for obscure/event cards too.
 */
...
async function getSeriesFromMedia(mediaUrl, timeout = MEDIA_TIMEOUT_MS) {
  ...
  try {
    ...
    if (textSeries) mediaSeriesCache.set(url, textSeries);
    return textSeries || null;
  } catch {
    return null;
  }
}
...
/**
 * Seed the name cache from series values that were already persisted in
 * MongoDB. This makes repaired collection data available immediately after a
 * process restart without re-querying AniList.
 */
export function rememberSeries(characterName, series) {
  const key = String(characterName || "").toLowerCase().trim();
  if (!key || !isKnownSeries(series)) return false;
  if (cache.get(key) === series) return false;
  cache.set(key, String(series).trim());
  markDirty();
  return true;
}
...
export async function getSeriesForCard(card, opts = {}) {
  const timeout = Number.isFinite(opts.timeout) ? Math.max(250, opts.timeout) : 2_500;
  const startedAt = Date.now();
  const ignoreStoredSeries = opts.ignoreStoredSeries === true;
  const forceLookup = opts.force === true;
  const mediaSeries = await getSeriesFromMedia(
    card?.media,
    Math.min(MEDIA_TIMEOUT_MS, timeout)
  );
  if (isKnownSeries(mediaSeries)) {
    const key = String(card?.name || "").toLowerCase().trim();
    if (key) rememberSeries(key, mediaSeries);
    return mediaSeries;
  }
  ...
  if (!forceLookup) {
    const cachedSeries = getSeriesCached(card?.name);
    if (isKnownSeries(cachedSeries)) return cachedSeries;
  }

  // Keep the command responsive when artwork has no embedded series metadata.
  // The lookup continues and updates the disk cache for the next card read.
  const remaining = Math.max(250, timeout - (Date.now() - startedAt));
  return getSeries(card?.name, { timeout: remaining, force: forceLookup })
    .then((series) => series || "Unknown");
}
