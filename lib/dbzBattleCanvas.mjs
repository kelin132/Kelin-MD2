...
async function loadImageSafe(url, label = "img") {
  ...
  // Try original first (may be a CDN PNG/WebP that Cairo can decode directly),
  // then PNG proxy (re-encodes to PNG, keeps alpha), then JPEG as safety net.
...
export async function generateBattleScene({ left, right, round = 1, hitSide = null, damage = null }) {
  ...
  // ── RIGHT fighter — top-right, flipped to face LEFT ───────────────────────
  const rCenterX = W * 0.72;
  const rGroundY = H * 0.42;

  drawGroundShadow(ctx, rCenterX, rGroundY, 80);
  ...
  if (rightImg) {
    ...
    ctx.fillText("👹", rCenterX, rGroundY - 100);
    ctx.textBaseline = "alphabetic";
  }

  // ── LEFT fighter — bottom-left, faces RIGHT ───────────────────────────────
  const lCenterX = W * 0.28;
  const lGroundY = H * 0.88;
  ...
  if (damage !== null && hitSide) {
    ...
    const cy = hitSide === "right" ? rGroundY - 240 : lGroundY - 280;
  ...
  ctx.textAlign = "center";
  ctx.fillText(left.character  || left.username  || "Fighter", lCenterX, lGroundY + 14);
  ctx.fillText(right.character || right.username || "Villain", rCenterX, rGroundY + 14);
