...
export async function downloadQuotedMedia(msg, allowedTypes = ["sticker", "image"]) {
  ...
  const media = mediaByType
    ...
    .map(([type, key]) => quoted[key] ? { message: quoted[key], type } : null)
    .find(Boolean);

  if (!media) {
    const errorCode = allowedTypes.includes("sticker") && allowedTypes.includes("image")
      ? "NOT_STICKER_OR_IMAGE"
      : "NOT_SUPPORTED_MEDIA";
...
export async function downloadQuotedStickerOrImage(msg) {
  return downloadQuotedMedia(msg, ["sticker", "image"]);
}
