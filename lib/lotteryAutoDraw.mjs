...
import { normalizeJid, bareJidNumber } from "./identity.mjs";
...
export function asJid(userId) {
  const value = String(userId || "");
  if (value.includes("@")) return normalizeJid(value);
  const digits = bareJidNumber(value);
  return digits ? `${digits}@s.whatsapp.net` : value;
}

export function mentionFor(userId) {
  const jid = asJid(userId);
  const number = bareJidNumber(jid);
  return number ? `@${number}` : "@player";
}
...
export function formatLotteryResults(result) {
  ...
  const rows = result.winners
    .map((winner) => `${mentionFor(winner.userId)} — $${winner.prize}`)
    .join("\n");
