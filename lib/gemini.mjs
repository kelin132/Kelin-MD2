...
import { princeApiText } from "./princeAPI.mjs";
...
// Fast Akira fallback. This is separate from askGemini so the existing
// .gemini command keeps using Google's official API when configured.
export async function askPrinceGemini(prompt) {
  // The PrinceTech endpoint expects a compact q value. Sending the full
  // persona prompt/history makes this provider reject otherwise valid calls.
  return princeApiText("ai/geminiaipro", { q: prompt.trim() }, 12_000);
}
