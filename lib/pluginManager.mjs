...
export async function routeMessage(sock, msg, prefix = ".", ownerNumber = "", fromMe = false) {
  ...
  // Keep the shared public avatar current even before a user registers on AIDORU.
  // This is best-effort and never blocks command execution if WhatsApp hides a photo.
  ...
  // Keep DMs owner-only for administrative commands. Website credential
  // commands are intentionally available in a user's private chat.
  if (isDM && !isOwner && cmd !== "wpw" && cmd !== "id") return;
