...
import { buildEconomyExternalAdReply, hasEconomyPreview } from "./economyPreview.mjs";
        ...
        get(target, property) {
          ...
          return async (chatId, content = {}, options) => {
            if (!previewSent && content && typeof content === "object" && typeof content.text === "string" && !content.linkPreview) {
              ...
              const externalAdReply = await buildEconomyExternalAdReply(plugin.name);
              if (externalAdReply) {
                content = {
                  ...content,
                  contextInfo: { ...(content.contextInfo || {}), externalAdReply },
                };
                previewSent = true;
                try {
                  const previewSend = target.sendMessage.call(target, chatId, content, options);
                  const timeout = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error("preview send timed out")), 3000).unref?.();
                  });
                  return await Promise.race([previewSend, timeout]);
                } catch (previewError) {
                  console.warn(`[${plugin.name}] rich preview failed; retrying plain message:`, previewError?.message || previewError);
                  return target.sendMessage.call(target, chatId, originalContent, options);
                }
              }
            }
