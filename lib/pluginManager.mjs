...
export async function routeMessage(sock, msg, prefix = ".", ownerNumber = "", fromMe = false) {
  ...
  // Match the exact command the user typed. Aliases such as `.p` should not
  // inherit a website thumbnail configured for `.profile`.
  // Leaderboards must return immediately. Building a website thumbnail here
  // can be noticeably slow on the first request. Only the wallet movement
  // aliases requested by the user get the automatic coin/link preview;
  // .mart builds its own preview explicitly.
  const shouldAttachPreview = new Set(["dep", "wd"]).has(cmd);
        ...
        get(target, property) {
          ...
          return async (chatId, content = {}, options) => {
            if (!previewSent && content && typeof content === "object" && typeof content.text === "string" && !content.linkPreview) {
              const config = getEconomyPreviewConfig(cmd);
              const linkPreview = await buildEconomyLinkPreview(cmd);
