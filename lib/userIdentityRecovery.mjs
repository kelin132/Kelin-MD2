...
async function findUser(db, id) {
  ...
  const exact = await users.findOne({ _id: String(id || "").trim() });
  if (exact) return exact;
  return users.findOne({ _id: { $in: values.slice(1) } });
}

function batchTargetId(value) {
  const source = String(value || "").trim();
  if (!source || /@(lid|g\.us|broadcast)$/.test(source)) return "";

  const isPhoneIdentity =
    /@(?:s\.whatsapp\.net|c\.us)$/.test(source) ||
    /^\+?\d[\d\s().:-]*$/.test(source);
  if (!isPhoneIdentity) return "";

  const number = normalizePhoneNumber(source);
  const target = phoneJid(number);
  return target && target !== source ? target : "";
}

async function discoverBatchPairs(db) {
  ...
  for (const user of users) {
    const sourceId = String(user._id || "").trim();
    const targetId = batchTargetId(sourceId);
    ...
    pairs.push({
      sourceId,
      targetId,
      legacySummary: plan.legacySummary,
  ...
  return pairs.sort((a, b) =>
    `${a.targetId}:${a.sourceId}`.localeCompare(`${b.targetId}:${b.sourceId}`)
  );
}
...
/**
 * Create a dry-run recovery plan for every legacy phone/device identity.
 * LIDs are intentionally skipped because they cannot be mapped to a phone
 * identity without WhatsApp's runtime metadata.
 */
export async function previewAllUserIdentityRecoveries() {
  const db = await getDb();
  const pairs = await discoverBatchPairs(db);
...
/**
 * Apply every discovered recovery sequentially. Sequential writes prevent
 * multiple legacy records for the same phone number from racing each other.
 * A failure is recorded and the remaining users still get a chance to restore.
 */
export async function restoreAllUserIdentities() {
  const plan = await previewAllUserIdentityRecoveries();
  const restored = [];
  const failed = [];

  for (const candidate of plan.candidates) {
    try {
      restored.push(await restoreUserIdentity(candidate.sourceId, candidate.targetId));
    } catch (error) {
      failed.push({
