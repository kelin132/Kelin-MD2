...
import { getSeriesCached } from "./seriesEnrich.mjs";

const API_BASE = "https://cardapi.eclipse.name.ng/api/cards";
const CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour
const API_FETCH_TIMEOUT_MS = 15_000;  // 15 seconds — prevents hanging forever
...
/**
 * Fetch a single tier from the API with a timeout so it never hangs.
...
export async function fetchAllCards() {
  ...
  // If every tier failed the API is completely down — tell callers clearly.
  if (successes.length === 0) {
    const firstErr = results.find(r => r.status === "rejected")?.reason;
    const err = new Error(
      `Card API is currently unreachable — please try again later. (${firstErr?.message ?? "timeout"})`
    );
    err.code = "CARD_API_DOWN";
    throw err;
  }

  const cards = successes.flatMap(r => r.value.map((raw, index) => normalise(raw, index + 1)));
 ...
 * Uses the in-memory series cache when available (primed from saved cards or
 * populated by a previous card lookup).
 */
...
export async function getCardsByTier(tier) {
  ...
  return all
    .filter(c => c.tierNum === num)
    .map((card) => {
      if (card.series === "Unknown") {
        const cached = getSeriesCached(card.name);
        if (cached) card.series = cached;
      }
      return card;
    });
}
