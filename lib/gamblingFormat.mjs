export function formatMoney(value) {
  ...
  if (n >= 1e12) return `${sign}${trimUnit(n / 1e12)}T ryu`;
  if (n >= 1e9) return `${sign}${trimUnit(n / 1e9)}B ryu`;
  if (n >= 1e6) return `${sign}${trimUnit(n / 1e6)}M ryu`;
  if (n >= 1e3) return `${sign}${trimUnit(n / 1e3)}K ryu`;
  return `${sign}${n.toLocaleString()} ryu`;
}
