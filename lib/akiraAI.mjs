/**
 * KELIN MD — Akira AI core (powered by David Cyril Kimi K2.6 API — no key needed)
 * Shared between the .akira command plugin and the group auto-trigger handler.
 * uid-based sessions keep per-chat conversation context.
 */

import { askKimiK26, askGeminiDC } from "./davidcyrilAPI.mjs";

// Per-chat uid map — reset clears the uid so Dolphin starts a fresh session
...
/** Fully reset a chat's Akira session — clears the jid→uid mapping so Dolphin starts fresh. */
export function resetAkiraSession(jid) {
  chatHistory.delete(jid);
}

/**
 * Build the full prompt sent to Dolphin.
 * The API has no native system-prompt param, so we prepend a condensed Akira
 * persona header to every message so the model stays in character.
 *
 * @param {string} userText
 * @param {boolean} forceShort – if true, adds a short-reply constraint
 */
function buildPrompt(userText, forceShort = false) {
  const shortNote = forceShort
    ? "\n[REPLY IN ONE SHORT SENTENCE ONLY. No more. Stay in character.]"
    : "";

  return `[SYSTEM — stay in character for the entire reply, no matter what]
${AKIRA_SYSTEM}${shortNote}
[END SYSTEM]
...
/**
 * Call the David Cyril Dolphin API as Akira and reply.
 * No API key required.
 *
 * Behaviour:
 *   • 30% chance of a forced one-sentence short reply
 *   • Always @tags the sender in the reply (mentions array)
 *
...
export async function callAkira(sock, msg, userText) {
  ...
  // 30% chance of a short one-sentence reply
  const forceShort = Math.random() < 0.30;

  await sock.sendPresenceUpdate("composing", jid);

  try {
    const prompt = buildPrompt(userText, forceShort);

    // Try Kimi K2.6 first — fall back to Gemini if it fails
    let reply;
    try {
      reply = await askKimiK26(prompt, uid);
    } catch {
      // Kimi K2.6 unavailable — use Gemini with the same persona prompt
      reply = await askGeminiDC(prompt, uid);
    }

    await sock.sendPresenceUpdate("paused", jid);

    // Tag the person Akira is replying to (groups only — DMs don't support mentions)
