...
export async function callAkira(sock, msg, userText) {
  ...
  try {
    return await runForUser(userJid, async () => {
      ...
      const prompt = [
        "CURRENT USER MESSAGE. Answer this message and nothing else first:",
        "<<<",
        userText.trim(),
        ">>>",
        replyHint(detailed, mode),
        "Reminder: do not answer an older message from the conversation history. React to the CURRENT USER MESSAGE above.",
      ].join("\n");
