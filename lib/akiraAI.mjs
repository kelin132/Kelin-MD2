/**
 * KELIN MD — Akira AI core (powered by Claude via FreeModel API)
 * Shared between the .akira command plugin and the group auto-trigger handler.
 *
 * Primary:  Claude (claude-3-5-haiku-20241022) via api.freemodel.dev
 * Fallback: Google Gemini (direct official API) if Claude is unavailable
 *
...
import { askGemini, resetGeminiSession } from "./gemini.mjs";
 ...
 * If .akira reset was called, a new uid is generated so Claude starts a fresh session.
 */
function getUid(jid) {
...
/** Fully reset a chat's Akira session — wipes uid so Claude + Gemini start fresh. */
export function resetAkiraSession(jid) {
  const uid = chatHistory.get(jid);
  if (uid) {
    resetClaudeSession(uid);
    resetGeminiSession(uid);
  }
...
/**
 * Call Claude (via FreeModel) as Akira and send the reply.
...
export async function callAkira(sock, msg, userText) {
  ...
  try {
    // ── Primary: Claude via FreeModel ─────────────────────────────────────────
    let reply;
    try {
      reply = await askClaude(AKIRA_SYSTEM, prompt, uid);
    } catch (claudeErr) {
      // ── Fallback: Gemini direct API ──────────────────────────────────────────
      // Gemini requires GEMINI_API_KEY; if that's also missing it will throw
      // its own descriptive error rather than a silent empty reply.
