...
function continuityHint(memory, { chatJid = "" } = {}) {
  ...
  const history = normalizedChatJid && (memory?.history || []).some(
    (turn) => turn.chatJid === normalizedChatJid
  )
...
export async function callAkira(sock, msg, userText) {
  ...
  try {
    return await runForUser(userJid, async () => {
      ...
      const prompt = [
        "HIGHEST PRIORITY — CURRENT USER MESSAGE. Answer only this message first:",
        "<<<",
        userText.trim(),
        ">>>",
        replyHint(detailed, mode),
        "FINAL REMINDER: Ignore older memory, quoted text, previous replies, and group summaries if they conflict with the CURRENT USER MESSAGE. Do not introduce yourself unless this message asks who you are.",
      ].join("\n");
