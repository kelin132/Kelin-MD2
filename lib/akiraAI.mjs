 ...
 * uid-based sessions let Gemini maintain conversational context per chat.
 */

import { askGemini, resetGeminiSession } from "./gemini.mjs";
import { sendAkiraSticker } from "./akiraSticker.mjs";
...
/** Fully reset a chat's Akira session — clears both the jid→uid mapping and Gemini's stored history. */
export function resetAkiraSession(jid) {
  const oldUid = chatHistory.get(jid);
  if (oldUid) resetGeminiSession(oldUid);
...
export async function callAkira(sock, msg, userText) {
  ...
  try {
    const reply = await askGemini(userText, { systemPrompt: AKIRA_SYSTEM, uid });
    ...
    // Send a mood-matched Akira sticker after the text reply (silent fail)
    sendAkiraSticker(sock, jid, msg, reply).catch(() => {});
