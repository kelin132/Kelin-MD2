...
async function getCanvasModule() {
  _canvasMod ??= import("@napi-rs/canvas");
  return _canvasMod;
}
...
function getSpriteUrl(fighter) {
  ...
  const currentForm = fighter.forms?.[fighter.currentFormIndex];
  const firstForm = fighter.forms?.find((form) => form?.imageUrl || form?.image);
  return normalizeImageUrl(fighter.imageUrl) ||
    normalizeImageUrl(currentForm?.imageUrl || currentForm?.image) ||
    normalizeImageUrl(firstForm?.imageUrl || firstForm?.image);
}
...
async function loadImageSafe(url, label = "img") {
  ...
  const attempts = [normalized, toPngProxy(normalized, 420), toJpegProxy(normalized, 420)];
...
export async function generateBattleScene({
  ...
  const [playerImg, enemyImg] = await Promise.all([
    loadImageSafe(getSpriteUrl(player), player.name),
    loadImageSafe(getSpriteUrl(enemy),  enemy.name),
  ]);
...
export async function generateTransformScene({ fighter, fromFormName, toFormName }) {
  ...
  // Fighter sprite
  const img = await loadImageSafe(getSpriteUrl(fighter), fighter.name);
...
export async function generateBattleResult({ winner, loser, rewardText = "", outcome = "victory" }) {
  ...
  const [wImg, lImg] = await Promise.all([
    loadImageSafe(getSpriteUrl(winner), winner.name),
    loadImageSafe(getSpriteUrl(loser),  loser.name),
  ]);
...
export async function generateVillainArrivalScene({ villain, level, kiFlavorText, fleeTimerMin = 30, isBoss = false }) {
  ...
  const img = await loadImageSafe(getSpriteUrl(villain), villain.name);
  const bounds = await getVisibleBounds(img);
...
export async function generateCharacterSelectCanvas(characters, page = 1, perPage = 8) {
  ...
  const images = await Promise.all(displayChars.map(c => loadImageSafe(getSpriteUrl(c), c.name)));
