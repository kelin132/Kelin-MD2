...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    // Get owner number from environment first, fallback to settings
    const ownerFromEnv = process.env.OWNER_NUMBER?.split(':')[0].split('@')[0];
    const ownerFromSettings = settings.ownerNumber?.split(':')[0].split('@')[0];
    const ownerNumber = ownerFromEnv || ownerFromSettings || '';
    
    if (!ownerNumber) {
        console.warn('⚠️  [isOwner] No owner number configured');
        return false;
    }

    const ownerJid = ownerNumber + "@s.whatsapp.net";
    const ownerNumberClean = ownerNumber;
