...
import { ANIME_QUIZ_QUESTIONS } from "./animeQuizQuestions.mjs";
...
/**
 * Fetch a question that hasn't been asked yet in this session.
 * Once the queue is empty, refill it with a fresh shuffle. This means a
 * question cannot repeat until every question in the bank has been used.
 */
async function fetchUniqueQuestion(session) {
  if (session.questionQueue.length === 0) {
    session.questionQueue = shuffleQuestions(ANIME_QUIZ_QUESTIONS);
    session.askedQuestions.clear();
  }

  const question = session.questionQueue.shift();
  session.askedQuestions.add(question.question);
  return question;
}

/** Map a letter (a/b/c/d) to the full option text, or return null. */
function resolveLetterAnswer(letter, options) {
  if (!options || !options.length) return null;
  const idx = { a: 0, b: 1, c: 2, d: 3 }[letter.toLowerCase()];
...
export async function openQuizLobby(sock, jid, starterJid, getUser) {
  ...
  const session = {
    ...
    askedQuestions: new Set(),         // question texts used in the current bank cycle
    questionQueue: shuffleQuestions(ANIME_QUIZ_QUESTIONS),
    question:      null,
    answer:        null,
    options:       [],
    lobbyTimer:    null,
...
async function sendNextQuestion(sock, jid) {
  ...
  // s.answer stores the full text so it remains valid after any shuffle.
...
export async function handleAnimeQuizText(sock, msg, getUser, saveUser) {
  ...
  // Allow single-letter answers: A/B/C/D → resolve to the full option text
  const given = (raw.length === 1 && /^[a-d]$/.test(raw))
    ? (resolveLetterAnswer(raw, s.options) ?? raw)
    : raw;
