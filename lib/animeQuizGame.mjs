 ...
 * Retries up to MAX_RETRIES times, then falls back to the last fetched
 * question even if it was repeated (better than crashing the game).
 */
async function fetchUniqueQuestion(session, maxRetries = 8) {
  let last = null;
  for (let i = 0; i < maxRetries; i++) {
    const q = await fetchQuestion();
    if (!session.askedQuestions.has(q.question)) {
      session.askedQuestions.add(q.question);
      return q;
    }
    last = q; // repeated — keep trying
  }
  // All retries were duplicates — use the last one anyway
  if (last) session.askedQuestions.add(last.question);
  return last;
}
...
export async function openQuizLobby(sock, jid, starterJid, getUser) {
  ...
  const session = {
    ...
    askedQuestions: new Set(),         // question texts already used this session
    question:      null,
    answer:        null,
    options:       [],
    lobbyTimer:    null,
