...
/**
 * Fetch image bytes ourselves and hand loadImage() a Buffer.
 * node-canvas's loadImage(url) is unreliable across builds when given
 * a raw http(s) string — this is far more consistent, and logs *why*
 * a portrait failed so it's debuggable from the bot's console output.
 */
async function loadImageSafe(url, label = "image") {
  if (!url) return null;
  try {
    ...
    if (!res.ok) {
      console.warn(`[battleCanvas] ${label} fetch failed: HTTP ${res.status} — ${url}`);
      return null;
    }
    const buf = Buffer.from(await res.arrayBuffer());
    return await loadImage(buf);
  } catch (err) {
    console.warn(`[battleCanvas] ${label} load error: ${err.message} — ${url}`);
    return null;
  }
}
...
async function drawPortrait(ctx, imageUrl, x, y, size, ringColor, dim = false, label = "fighter") {
  const img = await loadImageSafe(imageUrl, label);
...
export async function generateBattleScene({ left, right, round = 1, hitSide = null, damage = null, crit = false }) {
  ...
  // Left fighter
  await drawPortrait(ctx, left.imageUrl, 70, topY, portraitSize, "#ff9d2e", false, left.username);
  ctx.textAlign = "left";
  ctx.fillStyle = "#fff";
  ...
  await drawPortrait(ctx, right.imageUrl, rightX, topY, portraitSize, "#4d7dff", false, right.username);
  ctx.textAlign = "right";
  ctx.fillStyle = "#fff";
...
export async function generateResultScene({ winner, loser, rewardText = "", outcome = "victory" }) {
  ...
  const y = 130;
  await drawPortrait(ctx, winner.imageUrl, x, y, size, "#ffd54a", false, winner.username);
  ctx.fillStyle = "#fff";
  ...
  const lSize = 130;
  await drawPortrait(ctx, loser.imageUrl, width - lSize - 60, height - lSize - 70, lSize, "#666", true, loser.username);
  ctx.fillStyle = "#999";
