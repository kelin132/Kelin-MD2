 ...
 * There is intentionally no provider or key fallback.
 */
...
function getKey() {
  ...
  if (!key) {
    throw new Error(
      "FREEMODEL_API_KEY is not set. Add your FreeModel API key to the deployment environment.",
    );
  }
  return key;
}
...
export async function freeModelChat(messages, model = "gpt-4o-mini") {
  ...
  const res = await fetch(`${BASE}/chat/completions`, {
    ...
    // Keep Akira responsive: the caller can immediately use its fallback.
