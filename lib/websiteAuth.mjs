...
function whatsappIdentityVariants(value) {
  ...
  const withoutDevice = normalized.replace(/:\d+(?=@)/, "");
  const [local = "", domain = "s.whatsapp.net"] = withoutDevice.split("@");
...
export async function getPendingWebsiteCode(jid) {
  ...
  if (!user) throw new Error("You need to register first with .register <your_name>.");

  const candidates = [
    {
      code: user.websiteVerificationCode,
      expiresAt: user.websiteVerificationExpiresAt,
      kind: "verification",
    },
    { code: user.websiteResetCode, expiresAt: user.websiteResetExpiresAt, kind: "reset" },
  ];
  const pending = candidates.find(({ code, expiresAt }) => {
    const expiry = new Date(expiresAt ?? "").getTime();
    return /^\d{6}$/.test(String(code ?? "")) && Number.isFinite(expiry) && expiry > Date.now();
  });
  if (!pending) {
    const requestedAt = new Date(user.websiteOtpRequestedAt ?? "").getTime();
    const requestedExpiry = new Date(user.websiteOtpExpiresAt ?? "").getTime();
    if (
      typeof user.websiteId === "string" &&
      /^AID-[0-9A-F]{10}$/.test(user.websiteId) &&
      ...
      const created = await users().findOneAndUpdate(
        {
          ...
          // The website may serialize this timestamp differently from the
          // MongoDB driver. The expiry is the authoritative request window.
          websiteOtpRequestedAt: { $exists: true },
          websiteOtpExpiresAt: user.websiteOtpExpiresAt,
        },
        {
          $set: {
            websiteOtpHash: hashOtp(user.websiteId, otp, saltHex),
            websiteOtpSalt: saltHex,
            websiteOtpCreatedAt: new Date(),
          },
          $unset: {
      ...
      if (!created) {
        throw new Error("Your reset request changed. Press Forgot password again and retry.");
      }
      return {
        code: otp,
    ...
    throw new Error("No active website code was found. Start sign-in or password recovery first.");
  }
  return {
