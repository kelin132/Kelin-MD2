/**
 * KELIN MD — Dungeon Crawler Canvas Generator
...
/** Simple vector adventurer icon, styled by class. */
function drawHero(ctx, cx, cy, size, classKey) {
  ...
  if (classKey === "warrior") {
    ...
    ctx.fillStyle = "#9fe3f5";
    ctx.shadowColor = "#9fe3f5";
...
function drawEnemy(ctx, cx, cy, size, isBoss) {
  ctx.save();
  ctx.translate(cx, cy);
  const color = isBoss ? "#ff4747" : "#9a5cff";
  ctx.shadowColor = color;
  ctx.shadowBlur = size * (isBoss ? 0.6 : 0.35);
...
/* ────────────────────────────────────────────────────────────────────── */
export async function generateCharacterCard({ character }) {
  ...
  fillText(ctx, character.name, 200, 70, { font: "bold 30px Sans", color: "#fff" });
  fillText(ctx, `Lv.${character.level} ${character.class[0].toUpperCase() + character.class.slice(1)}`, 200, 98, { font: "16px Sans", color });

  drawHero(ctx, 100, 200, 75, character.class);

  const barX = 200, barW = 560;
  fillText(ctx, `HP  ${character.hp}/${character.maxHp}`, barX, 140, { font: "bold 13px Sans", color: "#ff8080" });
  drawBar(ctx, barX, 148, barW, 16, character.hp / character.maxHp, "#ff4747");

  const xpNeed = 50 * character.level;
  fillText(ctx, `XP  ${character.xp}/${xpNeed}`, barX, 182, { font: "bold 13px Sans", color: "#9fe3f5" });
  drawBar(ctx, barX, 190, barW, 16, character.xp / xpNeed, "#38a3ff");
  ...
  stats.forEach(([label, val], i) => {
    ...
    const y = 240 + row * 46;
    roundRect(ctx, x, y, colW - 16, 36, 8);
    ctx.fillStyle = "rgba(255,255,255,0.05)";
    ctx.fill();
    fillText(ctx, label, x + 14, y + 23, { font: "13px Sans", color: "#9aa4b2" });
    fillText(ctx, String(val), x + colW - 30, y + 24, { font: "bold 16px Sans", color: "#fff", align: "right" });
  });
...
export async function generateRoomCard({ type, roomNumber, totalRooms, character, detail }) {
  const { createCanvas } = await getCanvas();
  const W = 800, H = 420;
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext("2d");
  ...
  fillText(ctx, `ROOM ${roomNumber}/${totalRooms}`, W / 2, 70, { font: "bold 20px Sans", color: "#c8a06a", align: "center" });
...
/* ────────────────────────────────────────────────────────────────────── */
export async function generateBattleCard({ character, enemy, lastLine }) {
  const { createCanvas } = await getCanvas();
  const W = 900, H = 480;
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext("2d");

  bgGradient(ctx, W, H, enemy.isBoss ? "#240707" : "#12070f", "#030201");
  torchParticles(ctx, W, H, 40);

  fillText(ctx, enemy.isBoss ? "👑 BOSS BATTLE" : "⚔️ BATTLE", W / 2, 44, {
    font: "bold 26px Sans",
    color: enemy.isBoss ? "#ff4747" : "#ff9f43",
    align: "center",
  });

  drawHero(ctx, 170, 260, 90, character.class);
  drawEnemy(ctx, W - 170, 260, enemy.isBoss ? 110 : 85, enemy.isBoss);

  fillText(ctx, character.name, 170, 380, { font: "bold 18px Sans", color: "#fff", align: "center" });
  drawBar(ctx, 60, 392, 220, 16, character.hp / character.maxHp, "#50EF39");
  fillText(ctx, `${character.hp}/${character.maxHp}`, 170, 424, { font: "13px Sans", color: "#cfd6e0", align: "center" });

  fillText(ctx, enemy.name, W - 170, 380, { font: "bold 18px Sans", color: "#fff", align: "center" });
  drawBar(ctx, W - 280, 392, 220, 16, enemy.hp / enemy.maxHp, "#ff4747");
  fillText(ctx, `${enemy.hp}/${enemy.maxHp}`, W - 170, 424, { font: "13px Sans", color: "#cfd6e0", align: "center" });

  fillText(ctx, "VS", W / 2, 260, { font: "bold 28px Sans", color: "#665", align: "center" });

  if (lastLine) {
    roundRect(ctx, W / 2 - 260, H - 46, 520, 34, 10);
...
export async function generateResultCard({ success, character, roomsCleared, meta = {} }) {
  ...
  drawHero(ctx, W / 2, 220, 90, character.class);
...
export async function generateLeaderboardCard({ entries }) {
  ...
  fillText(ctx, "( ranked by level )", W / 2, 78, { font: "13px Sans", color: "#8891a0", align: "center" });
  ...
  entries.forEach((e, i) => {
    ...
    fillText(ctx, `${e.name} (${e.class})`, 100, y + 30, { font: "bold 16px Sans", color: "#fff" });
