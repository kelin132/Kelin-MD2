...
import { phoneJid } from "./whatsappIdentity.mjs";
...
function numberRegex(number) {
  return new RegExp(`^\\+?${number}(?::\\d+)?(?:@|$)`);
}
...
export async function findRegisteredWebIdentity(jids) {
  ...
  for (const number of numbers) {
    const byNumber = await users.findOne({
      registered: true,
      $or: [
        { _id: { $regex: numberRegex(number) } },
        ...(Number.isSafeInteger(Number(number)) ? [{ _id: Number(number) }] : []),
      ],
    });
...
export async function createWebLinkCode(jid, aliases = []) {
  ...
  const canonicalJid = phoneJid(identifier) || normalizedJid;

  if (!canonicalJid || !identifier) {
  ...
  await collection.insertOne({
    ...
    jids,
    jidAliases: jids.filter((candidate) => candidate !== canonicalJid),
    createdAt: now,
    expiresAt,
    usedAt: null,
  });
