 ...
 *
 * Every 48 hours, all registered users are taxed 30% of their wallet (money)
 * and 30% of their bank balance.  The deduction is applied atomically with
...
const TAX_RATE_PERCENT = 30;           // % taken from wallet + bank
...
async function collectTax() {
  const now = Date.now();
  log("info", "[tax] 🏛️  Collecting 30% tax from all registered members...");
  ...
  try {
    ...
    // Apply 30% deduction atomically using $mul (multiply by 0.7)
    const multiplier = 1 - TAX_RATE_PERCENT / 100; // 0.70
