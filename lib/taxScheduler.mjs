 ...
 *
 * Every 48 hours, all registered users are taxed 15% of their wallet (money)
 * and 15% of their bank balance.  The deduction is applied atomically with
...
const TAX_RATE_PERCENT = 15;           // % taken from wallet + bank
...
async function collectTax() {
  const now = Date.now();
  log("info", "[tax] 🏛️  Collecting 15% tax from all registered members...");
  ...
  try {
    ...
    // Apply 15% deduction atomically using $mul (multiply by 0.85)
    const multiplier = 1 - TAX_RATE_PERCENT / 100; // 0.85
