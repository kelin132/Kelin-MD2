/**
 * KELIN MD — configurable AI persona auto-trigger handler
 * Fires when someone:
 *   1. @mentions / @tags the bot
 *   2. Replies to any message sent by the bot (groups + DMs)
 *   3. Writes the active persona's name or alias anywhere in the message
 *      (25% chance the persona ignores a name-only trigger)
 *
 * Works in both groups AND DMs.
 * Skips commands (prefix-leading messages).
 */
import { callAkira } from "./akiraAI.mjs";
import { getActivePersona, getPersonaTriggerNames } from "./aiPersonas.mjs";
...
export async function akiraHandler({ sock, msg, prefix = "." }) {
  ...
  // ── Trigger 2: active persona name or configured alias appears anywhere ──
  const lowerBody = body.toLowerCase();
  const containsName = getPersonaTriggerNames().some((name) =>
  ...
  // ── Random skip: name-only trigger gets 25% chance of being ignored ───────
