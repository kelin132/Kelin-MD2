...
/**
 * Download YouTube audio directly from the bot host as a fallback when a
 * provider returns an IP-bound signed URL that responds with HTTP 403.
 */
export async function downloadYoutubeAudio(videoUrl, { timeoutMs = DEFAULT_MEDIA_TIMEOUT, maxBytes = MAX_MEDIA_BYTES } = {}) {
  ...
  try {
    ...
    throw new Error("The local YouTube audio fallback is not installed");
  }
  ...
  const timeout = new Promise((_, reject) => {
    setTimeout(() => reject(new Error(`Direct YouTube audio timed out after ${timeoutMs}ms`)), timeoutMs);
  });

  let info;
  try {
    info = await Promise.race([
      ytdl.getInfo(videoUrl, { requestOptions: { headers } }),
      timeout,
    ]);
  } catch (error) {
    throw new Error(`Direct YouTube audio unavailable: ${error.message}`);
  }
  ...
  if (!format?.url) throw new Error("YouTube returned no audio-only format");

  const stream = ytdl.downloadFromInfo(info, { format, requestOptions: { headers } });
  const chunks = [];
  let size = 0;
  const timer = setTimeout(() => stream.destroy(new Error(`Direct YouTube audio timed out after ${timeoutMs}ms`)), timeoutMs);
  try {
    for await (const chunk of stream) {
      size += chunk.length;
      if (size > maxBytes) {
        stream.destroy(new Error(`Direct YouTube audio is too large (${Math.ceil(size / 1024 / 1024)} MB)`));
        throw new Error(`Direct YouTube audio is too large (${Math.ceil(size / 1024 / 1024)} MB)`);
      }
      chunks.push(chunk);
    }
  } catch (error) {
    throw new Error(`Direct YouTube audio fetch failed: ${error.message}`);
  } finally {
    clearTimeout(timer);
  }

  const buffer = Buffer.concat(chunks);
  if (!buffer.length) throw new Error("YouTube returned an empty audio file");
  return {
    buffer,
    mimetype: format.mimeType?.split(";")[0] || "audio/mp4",
    size: buffer.length,
    title: info.videoDetails?.title || "YouTube Audio",
    extension: format.container || "m4a",
  };
}
