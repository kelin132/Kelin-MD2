...
async function createRoomCode(rooms) {
  let code = roomCodeFromId(`battle-${Date.now()}-${Math.random()}`);
  while (await rooms.findOne({ code })) {
    code = roomCodeFromId(`battle-${Date.now()}-${Math.random()}`);
  }
  return code;
}
...
async function createWebBattleRoomInternal({
  ...
  if (existing) {
    if (!existing.code) existing.code = await createRoomCode(rooms);
