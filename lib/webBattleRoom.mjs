...
async function createRoomCode(rooms) {
  let code = roomCodeFromId(randomUUID());
  while (await rooms.findOne({ code })) {
    code = roomCodeFromId(randomUUID());
  }
  return code;
}
...
async function createWebBattleRoomInternal({
  ...
  const now = new Date();
  await rooms.deleteMany({ expiresAt: { $lte: now } });
  const hasOpponent = Boolean(opponentTrainer && Array.isArray(opponentParty));
  const existingFilter = {
    "challenger.id": challengerJid,
    status: { $in: ["waiting", "active"] },
    ...(opponentJid ? { invitedOpponentId: opponentJid } : {}),
  };
  const existing = await rooms.findOne(existingFilter);
  if (existing) {
    // Rooms created by older bot builds had no code. Backfill one before
    // returning them so an already-open challenge can still be shared by code.
    if (!existing.code) {
      existing.code = await createRoomCode(rooms);
    }
    // Older rooms may have been created before the invited trainer was
    // snapshotted. Hydrate that room when the challenge is repeated so the
    // link still opens a live arena instead of an orphaned waiting screen.
  ...
  const room = {
    ...
    code: await createRoomCode(rooms),
