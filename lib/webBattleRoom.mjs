...
async function createWebBattleRoomInternal({
  ...
  if (existing) {
    if (!existing.opponent && opponentTrainer && Array.isArray(opponentParty)) {
      ...
      await rooms.updateOne(
        { _id: existing._id },
        {
          ...
          $push: { combatLog: "The challenged trainer's party has been loaded into this room." },
        },
      );
      existing.opponent = opponentSnapshot;
      existing.invitedOpponentId = opponentJid || existing.invitedOpponentId || null;
      existing.autoStart = true;
      existing.combatLog = [...(existing.combatLog || []), "The challenged trainer's party has been loaded into this room."];
    }
    return existing;
  }
  ...
  const room = {
    ...
    opponent: opponentTrainer && Array.isArray(opponentParty)
      ? trainerSnapshot({
          trainer: opponentTrainer,
          party: opponentParty,
          jid: opponentJid,
    ...
    combatLog: [
      opponentTrainer
        ? "Room opened from a WhatsApp challenge. Both parties are loaded; waiting for the challenged trainer to enter."
