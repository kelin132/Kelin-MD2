...
export function normalizePhoneNumber(value = "") {
  ...
  const numberPart = raw.split("@")[0].split(":")[0];
  return numberPart
    .replace(/[^\d]/g, "")
    .replace(/^00/, "");
}
...
export function userJidFilter(value = "") {
  const raw = String(value || "").trim();
  if (!raw || /@(lid|g\.us|broadcast)$/.test(raw)) return { _id: raw };

  const number = jidNumber(raw);
  if (!number || (!isPhoneJid(raw) && !/^\+?\d[\d\s().-]*$/.test(raw))) {
    return { _id: raw };
  }
...
function phoneNumberFromParticipant(participant = {}) {
  ...
  return normalizePhoneNumber(phoneCandidate);
}
  ...
  try {
    ...
    for (const participant of metadata.participants || []) {
      ...
      if (ids.some((id) => candidates.includes(id) || candidateNumbers.has(jidNumber(id)))) {
        aliases.push(...ids);
        const participantPhone = phoneNumberFromParticipant(participant);
        if (participantPhone) aliases.push(phoneJid(participantPhone));
      }
    }
...
export async function resolveDisplayNumber(sock, storedJid, chatId) {
  ...
  if (chatId?.endsWith("@g.us") && sock?.groupMetadata) {
    try {
      const metadata = await sock.groupMetadata(chatId);
      for (const participant of metadata.participants || []) {
        const participantNumber = phoneNumberFromParticipant(participant);
