...
export function formatAkiraMemory(memory) {
  const history = memory.history
    .slice(-16)
    .map((turn) => `${turn.role === "user" ? "User" : "Akira"}: ${turn.text}`)
    .join("\n");
...
Use older turns to keep continuity: remember unfinished topics, preferences, and the emotional thread when relevant.
Recent conversation:
