    ...
    history: Array.isArray(doc.history)
      ? doc.history
          ...
          .map((turn) => ({
            ...
            ts: turn.ts || null,
            chatJid: normalizeJid(turn.chatJid),
          }))
...
export async function recordAkiraTurn(userJid, {
  ...
  const turns = [
    { role: "user", text: safeUserText, ts: Date.now(), chatJid: normalizeJid(chatJid) },
    { role: "assistant", text: safeAssistantText, ts: Date.now(), chatJid: normalizeJid(chatJid) },
  ];
...
export function formatAkiraMemory(memory, { chatJid = "" } = {}) {
  const normalizedChatJid = normalizeJid(chatJid);
  // Memory is stored per sender, but a sender may talk to Akira in multiple
  // groups. Never leak another group's conversation into this prompt.
  const chatHistory = normalizedChatJid
