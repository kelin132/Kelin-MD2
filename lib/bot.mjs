...
const MAX_PENDING_MESSAGES_PER_KEY = 8;        // Increased from 3 to 8
const MAX_CONCURRENT_COMMANDS_PER_KEY = 4;    // Increased from 2 to 4
          ...
          try {
            ...
            if (!isAfkCommand) {
              ...
              if (senderAfk) {
                ...
                getDb().then(db => db.collection("users").updateOne({ _id: senderJid }, { $set: { afk: null } })).catch(() => {});

                void sock.sendMessage(msg.key.remoteJid, {
                  text: `🌸 *@${tag}* is back online! (Away for ${timeStr})`,
                  mentions: [senderJid],
                }, { quoted: msg });
              }
            }
            ...
            for (const jid of mentionedJids) {
              ...
              if (afkData) {
                ...
                void sock.sendMessage(msg.key.remoteJid, {
                  text: `😴 *@${tag}* is AFK: _${afkData.reason}_`,
                  mentions: [jid],
                }, { quoted: msg });
              }
            }
