...
async function syncBotRegistry(jid, { online = false, startedAt = processStartedAt } = {}) {
  ...
  try {
    ...
    if (!registryUnavailableLogged) {
      log(
        "warn",
        `Bot registry unavailable: ${error.message}. WhatsApp will stay online and registry updates will retry.`,
      );
...
export async function connectBot(phoneNumber, prefix) {
  ...
  try {
    ...
    // Avoid Baileys fetching URL metadata for normal bot replies. A slow or
    // unreachable website can otherwise delay a send by the preview timeout.
    // Explicit linkPreview objects (used by economy previews) still pass
    // through unchanged.
    ...
    if (!authState.creds.registered && _phoneNumber) {
      ...
      try {
        ...
        showPairingCode(code, _phoneNumber);
          ...
          log("warn", "No QR fallback is enabled. Check the phone number and try again.");
      }
    }
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      // Deliberately do not print or fall back to QR codes. New accounts must
      // be linked with the pairing code; existing credential files reconnect
      // without any interactive login.
      if (update.qr) {
        log("warn", "WhatsApp offered a QR code, but QR login is disabled. Waiting for pairing-code login.");
      }
      ...
      if (connection === "close") {
        ...
        if (sessionReplaced) {
          log(
            "error",
            "WhatsApp replaced this session (440). Stop the other process using the same creds.json, then restart this bot.",
          );
          // The supervisor treats 42 as a terminal worker state and will not
          // start the same credentials again on its next scan.
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        try {
          ...
          // Route commands immediately. Giveaway reactions, game text, AFK
          // lookups, and mention notifications only apply to non-command
          // messages and must not sit in front of a command reply.
          if (isCommandMessage) {
