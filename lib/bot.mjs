...
function queueIncomingMessage(msg, handler) {
  ...
  if (queue?.pending >= MAX_PENDING_MESSAGES_PER_KEY) {
    const body =
  ...
  current.finally(() => {
    queue.pending -= 1;
    if (queue.pending === 0 && messageQueues.get(queueKey) === queue) messageQueues.delete(queueKey);
  }).catch(() => {});
}
