...
/**
 * Reconnect the WhatsApp socket without terminating the Node.js process.
 * Panel hosts treat process.exit() as a stopped application, so owner
 * initiated restarts must stay inside the connection manager.
 */
export function restartConnection() {
  ...
  if (!sock) {
    reconnectTimer = setTimeout(() => connectBot(null, _prefix), 1_000);
    return;
  }

  log("info", "Owner requested a WhatsApp connection restart.");
  if (typeof sock.end === "function") {
    sock.end(new Error("Owner requested connection restart"));
  } else if (typeof sock.ws?.close === "function") {
    sock.ws.close();
  } else {
    throw new Error("WhatsApp socket cannot be restarted safely");
  }
}
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        try {
          ...
          // with the moderation checks. Akira is intentionally not awaited
          // here: a slow or unavailable AI provider must not block commands,
          // moderation, or later WhatsApp messages.
