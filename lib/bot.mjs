...
// Normalize phone JIDs and preserve privacy LIDs (e.g. 123:12@lid).
// The old helper converted LIDs into invalid "123@lid@s.whatsapp.net" values,
// which made persisted AFK records impossible to find when the next message
// arrived.
const cleanJid = (jid = "") => normalizeJid(jid);
          ...
          try {
            ...
            if (!isAfkCommand && !msg.key.fromMe) {
              ...
              if (senderAfk) {
                ...
                // Use the economy helpers so legacy/LID identities are resolved
                // the same way they are when .afk saves the user.
                try {
                  await clearStoredAfk(senderJid);
                } catch (e) {
                  log("error", "Failed to clear AFK state from DB: " + String(e));
                }
                ...
                // Wait for the notification so a send failure is visible to the
                // message handler instead of becoming an unhandled void promise.
...
async function clearStoredAfk(rawJid) {
  const user = await getUser(cleanJid(rawJid));
  if (!user?.afk?.active) return false;

  user.afk = null;
  await saveUser(cleanJid(rawJid), user);
  return true;
}
