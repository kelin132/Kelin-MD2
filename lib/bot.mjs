...
let _ownerNumber   = null; // owner number persisted
...
export async function connectBot(phoneNumber, prefix, ownerNumber) {
  if (prefix) _prefix = prefix;
  if (phoneNumber) _phoneNumber = phoneNumber.replace(/\D/g, "");
  if (ownerNumber) _ownerNumber = ownerNumber.replace(/\D/g, "");
  ...
  try {
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        if (!loggedOut) {
          ...
          reconnectTimer = setTimeout(() => connectBot(null, _prefix, _ownerNumber), wait);
        } else {
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        try {
          ...
          await routeMessage(sock, msg, _prefix, _ownerNumber || process.env.OWNER_NUMBER, msg.key.fromMe === true);
        } catch (err) {
    ...
    if (reconnectTimer) clearTimeout(reconnectTimer);
    reconnectTimer = setTimeout(() => connectBot(null, _prefix, _ownerNumber), 10_000);
  }
}
