...
class GroupSettings {
  ...
  recordActivity(jid, sender) {
    ...
    activityCounts[sender] = (activityCounts[sender] || 0) + 1;
    const activityLastSeen = {
      ...(current.activityLastSeen || {}),
      [sender]: Date.now(),
    };
    cache[jid] = { ...current, activityCounts, activityLastSeen };
    mongoSet(jid, { activityCounts, activityLastSeen });
  }
