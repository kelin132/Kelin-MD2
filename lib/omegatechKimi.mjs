 ...
 * KELIN MD — AI helper
 *
 * Uses GiftedTech, OmegaTech, and PrinceTech public AI endpoints.
 */
...
const PRINCE_AI_PROVIDERS = [
  ...
  { name: "PrinceGPT-4", endpoint: "ai/gpt" },
 ...
 * @param {boolean} [opts.usePrinceChain] - Use the PrinceTech AI provider chain
...
export async function askKimi(prompt, opts = {}) {
  const {
    ...
    model = null,
    allowFallbacks = true,
    usePrinceChain = false,
  } = opts;
  const fullPrompt = systemPrompt ? `${systemPrompt}\n\n${prompt}` : prompt;

  const defaultProviders = [
    // 0. Prince Tech AI (User's preferred primary)
    {
      name: "Prince-Tech",
      type: "fetch",
      url: `https://api.princetechn.com/api/ai/gpt4o-mini?apikey=prince&q=${encodeURIComponent(fullPrompt)}`
    },

    // FreeModel is an OpenAI-compatible gateway
    ...(FREEMODEL_API_KEY ? [{
      name: "FreeModel",
      type: "openai",
      url: FREEMODEL_URL,
  ...
  const princeProviders = PRINCE_AI_PROVIDERS.map((provider) => ({
    ...provider,
    type: "prince",
  }));
  const providers = usePrinceChain ? princeProviders : defaultProviders;
