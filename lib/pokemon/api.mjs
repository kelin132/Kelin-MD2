...
const API_ORIGIN = "https://pokeapi.eclipse.name.ng";

/** Convert a relative API path to an absolute URL */
function toAbsoluteUrl(url) {
  if (!url) return url;
  if (url.startsWith("http://") || url.startsWith("https://")) return url;
  // Relative path like /api/pokemon/498/image
...
function normalizePokemon(raw) {
  ...
  // Image URL — try several common keys, then make absolute
...
export async function fetchByName(name) {
  ...
  // API returns { items: [...] }, a plain array, or a single object
  let data;
  ...
  if (!data) throw new Error(`No Pokémon found for "${name}"`);
  // Search results may be stubs (no stats) — fetch full data by ID
  if (!data.stats && data.id) return fetchById(data.id);
