...
export async function getImageMessage(pokemon) {
  ...
  const fallback = Number.isInteger(id) && id > 0
    ...
    : null;
  return imagePayload(pokemon?.imageLocalPath, [pokemon?.imageUrl, pokemon?.frontSpriteUrl, fallback]);
}
...
export async function getBackImageMessage(pokemon) {
  ...
  const fallback = Number.isInteger(id) && id > 0
    ...
    : null;
  return imagePayload(pokemon?.backImageLocalPath, [pokemon?.backImageUrl, pokemon?.backSpriteUrl, fallback]);
}
