 ...
 *  1. Local files  — downloaded by running `python3 download_pokeapi.py` in the
 *                    bot's root directory. Eliminates all external API calls and
 *                    CDN image failures.  Paths: ./data/api/v2/pokemon/ and
 *                    ./sprites/pokemon/
...
// These match where download_pokeapi.py extracts the archives
...
function normalizePokemon(raw) {
  ...
  // Keep the API's low-resolution sprites separate from the larger artwork.
  // Battle canvases use these deliberately so the two fighters share the
  // same classic pixel-art perspective.
  const pixelImageUrl =
    raw.sprites?.front_default ||
    (pid ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pid}.png` : null);
  const pixelBackImageUrl =
    raw.sprites?.back_default ||
    (pid ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/back/${pid}.png` : null);
  const pixelShinyImageUrl =
    raw.sprites?.front_shiny ||
    (pid ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/${pid}.png` : null);
  const pixelShinyBackImageUrl =
    raw.sprites?.back_shiny ||
    (pid ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/back/shiny/${pid}.png` : null);
...
export async function getImageMessage(pokemon) {
  if (pokemon?.imageLocalPath) {
    try {
      const buf = await readFile(pokemon.imageLocalPath);
      return { image: buf };
    } catch { /* fall through to URL */ }
  }
  if (pokemon?.imageUrl) {
    return { image: { url: pokemon.imageUrl } };
  }
  return null;
}
...
export async function getBackImageMessage(pokemon) {
  if (pokemon?.backImageLocalPath) {
    try {
      const buf = await readFile(pokemon.backImageLocalPath);
      return { image: buf };
    } catch { /* fall through */ }
  }
  if (pokemon?.backImageUrl) {
    return { image: { url: pokemon.backImageUrl } };
  }
  return null;
}

/**
 * Return battle sprite URLs in draw order.
 *
 * The player uses a back sprite and the opponent uses a front sprite. Shiny
 * URLs are preferred when the stored Pokémon is shiny, then regular pixel
 * sprites are used before any legacy artwork URLs.
 */
export function getBattleSpriteUrls(pokemon, side = "enemy") {
  const id = pokemon?.pokedexId ?? pokemon?.id;
  const front = pokemon?.pixelImageUrl ||
    (id ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png` : null);
  const back = pokemon?.pixelBackImageUrl ||
    (id ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/back/${id}.png` : null);
  const shinyFront = pokemon?.pixelShinyImageUrl ||
    (id ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/shiny/${id}.png` : null);
  const shinyBack = pokemon?.pixelShinyBackImageUrl ||
    (id ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/back/shiny/${id}.png` : null);
  const legacyFront = pokemon?.imageUrl;
  const legacyBack = pokemon?.backImageUrl;

  if (side === "player") {
    return [
      pokemon?.shiny ? shinyBack : null,
      back,
      pokemon?.shiny ? shinyFront : null,
      front,
      legacyBack,
      legacyFront,
    ].filter(Boolean);
...
export async function fetchById(id) {
  ...
  // ── 1. Try local data (downloaded by download_pokeapi.py) ──────────────────
