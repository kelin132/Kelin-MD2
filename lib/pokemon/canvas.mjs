...
import { getBattleSpriteUrls } from "./api.mjs";
...
function drawTrainerSprite(ctx, image, x, footY, size, flip = false) {
  if (!image) return;

  drawGroundShadow(ctx, x + size / 2, footY + 4, size * 0.34);
  pixelatedImage(ctx, image, x, footY - size, size, flip);
}

function pixelatedImage(ctx, image, x, y, size, flip = false) {
  ctx.save();
  // Nearest-neighbour scaling preserves the hard edges of PokeAPI's
  // low-resolution battle sprites when they are enlarged for the canvas.
...
// ── Arena helpers (classic handheld battle aesthetic) ─────────────────────────
...
function getBattlePlatformLayout(W, H) {
  return {
    player: { cx: W * 0.27, cy: H * 0.84, rx: 230, ry: 42 },
    enemy: { cx: W * 0.74, cy: H * 0.49, rx: 180, ry: 34 },
  };
}

/** Draw the soft grassy battle patches from the reference image. */
function drawBattleCircle(ctx, { cx, cy, rx, ry }, accent = "#a9ed78") {
  ctx.save();
  ctx.shadowColor = "rgba(54, 128, 56, 0.24)";
  ctx.shadowBlur = 5;
  ctx.shadowOffsetY = 5;

  const patch = ctx.createRadialGradient(cx, cy - 8, 10, cx, cy, rx);
  patch.addColorStop(0, "#d5ff9a");
  patch.addColorStop(0.6, accent);
  patch.addColorStop(1, "#67d45f");
  ctx.fillStyle = patch;
  ctx.beginPath();
  ...
  // Small blades make the otherwise simple ellipse read as a hand-drawn
  // grass patch while remaining crisp when enlarged from a low-res sprite.
  const rand = seededRand(Math.round(cx + cy));
  ctx.save();
  ctx.strokeStyle = "rgba(78, 190, 72, 0.72)";
  ctx.lineWidth = 3;
  ctx.lineCap = "round";
  for (let i = 0; i < 30; i++) {
    const x = cx - rx * 0.9 + rand() * rx * 1.8;
    const spread = Math.sqrt(Math.max(0, 1 - ((x - cx) / rx) ** 2));
    const y = cy - ry * spread * (0.35 + rand() * 0.45) + rand() * ry * 0.7;
    const h = 6 + rand() * 14;
    ctx.beginPath();
    ctx.moveTo(x, y + 5);
    ctx.lineTo(x + (rand() - 0.5) * 10, y - h);
    ctx.stroke();
  }
  ctx.restore();
}

/** Rocky terrain texture: angular stones, seams, and small loose pebbles. */
function drawRockyTerrain(ctx, W, H) {
  const rand = seededRand(318);
  ctx.save();
  ctx.globalAlpha = 0.46;

  for (let i = 0; i < 34; i++) {
    const x = rand() * W;
    const y = H * 0.49 + rand() * H * 0.48;
    const width = 22 + rand() * 72;
    const height = 5 + rand() * 16;
    const shade = 38 + Math.floor(rand() * 42);
    const points = 5 + Math.floor(rand() * 3);

    ctx.fillStyle = `rgb(${shade + 7},${shade + 9},${shade + 10})`;
    ctx.strokeStyle = `rgba(12,15,17,0.46)`;
    ctx.lineWidth = 1;
    ctx.beginPath();
    for (let p = 0; p < points; p++) {
      const angle = (Math.PI * 2 * p) / points;
      const radius = 0.72 + rand() * 0.34;
  ...
  for (let i = 0; i < 13; i++) {
    const x = rand() * W;
    const y = H * 0.53 + rand() * H * 0.4;
    const length = 22 + rand() * 64;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x + length * 0.42, y + (rand() - 0.5) * 10);
    ctx.lineTo(x + length, y + (rand() - 0.5) * 7);
    ctx.stroke();
  }

  ctx.globalAlpha = 0.65;
  ctx.fillStyle = "#9a9682";
  for (let i = 0; i < 48; i++) {
    const x = rand() * W;
    const y = H * 0.5 + rand() * H * 0.46;
...
/** Full battle backdrop: the bright striped grassland from the supplied reference. */
function drawArenaBackground(ctx, W, H) {
  const sky = ctx.createLinearGradient(0, 0, 0, H * 0.5);
  sky.addColorStop(0, "#b9e1ee");
  sky.addColorStop(0.55, "#d9ecdf");
  sky.addColorStop(1, "#d5f0d0");
  ctx.fillStyle = sky;
  ctx.fillRect(0, 0, W, H);

  // Horizontal scanline bands recreate the layered sky/grass texture.
  ctx.save();
  for (let y = 0; y < H; y += 17) {
    ctx.fillStyle = y < H * 0.46
      ? (y % 34 === 0 ? "rgba(255,255,255,0.42)" : "rgba(106,175,193,0.18)")
      : (y % 34 === 0 ? "rgba(255,255,255,0.28)" : "rgba(95,185,101,0.13)");
    ctx.fillRect(0, y, W, 5);
  }
  ctx.restore();

  // Pale field wash below the horizon.
  const field = ctx.createLinearGradient(0, H * 0.42, 0, H);
  field.addColorStop(0, "rgba(236, 249, 228, 0.35)");
  field.addColorStop(1, "rgba(177, 229, 166, 0.52)");
  ctx.fillStyle = field;
  ctx.fillRect(0, H * 0.42, W, H * 0.58);

  // Sparse pixel-like grass marks across the field.
  const rand = seededRand(81);
  ctx.save();
  ctx.strokeStyle = "rgba(117, 204, 108, 0.58)";
  ctx.lineWidth = 3;
  ctx.lineCap = "square";
  for (let i = 0; i < 95; i++) {
    const x = rand() * W;
    const y = H * 0.45 + rand() * H * 0.48;
    const h = 3 + rand() * 11;
    ctx.beginPath();
    ctx.moveTo(x, y + 4);
    ctx.lineTo(x + (rand() - 0.5) * 9, y - h);
    ctx.stroke();
  }
  ctx.restore();

  // Pokémon-style grassy battle patches: their center lines are sprite anchors.
  const platforms = getBattlePlatformLayout(W, H);
  drawBattleCircle(ctx, platforms.player, "#a3e873");
  drawBattleCircle(ctx, platforms.enemy, "#a9ed78");
}
...
/** Cream dialogue panel inspired by the reference game's battle prompt. */
function drawDialogueBox(ctx, x, y, w, h, text) {
  ctx.save();
  ctx.fillStyle = "#111";
  ...
  // A tiny warm inset keeps the panel from feeling digitally flat.
  ctx.strokeStyle = "rgba(190, 186, 128, 0.28)";
  ctx.lineWidth = 2;
  ctx.strokeRect(x + 7, y + 7, w - 14, h - 14);

  const message = String(text || "Choose a move!").slice(0, 118);
  ...
  lines.slice(0, 3).forEach((current, index) => {
    ctx.fillText(current, x + 18, firstY + index * lineHeight);
  });
  ctx.restore();
}
 ...
 * @param {object|null} [opts.playerTrainer]  { name, imagePath }
 * @param {object|null} [opts.enemyTrainer]   { name, imagePath }
...
export async function generateBattleScene({
  ...
  // Load the API's pixel sprites in parallel. The player gets a back sprite
  // and the enemy gets a front sprite so both fighters read as facing each
  // other. Legacy URLs remain at the end for old stored battle records.
  const [playerImg, enemyImg, playerTrainerImg, enemyTrainerImg] = await Promise.all([
    loadImageWithFallbacks(
      [
        ...getBattleSpriteUrls(player, "player"),
        player.imageUrl,
      ].filter(Boolean),
      player.name
    ),
    loadImageWithFallbacks(
      [
        ...getBattleSpriteUrls(enemy, "enemy"),
        enemy.imageUrl,
      ].filter(Boolean),
      enemy.name
    ),
    loadImageSafe(playerTrainer?.imagePath, playerTrainer?.name || "trainer"),
    loadImageSafe(enemyTrainer?.imagePath, enemyTrainer?.name || "trainer"),
  ]);

  // ── ENEMY sprite — anchored directly to the enemy battle circle ───────────
  const platforms = getBattlePlatformLayout(W, H);
  const enemySize = 240;
  const enemyCX = platforms.enemy.cx;
  const enemyFootY = platforms.enemy.cy + 2;
  ...
  if (enemyImg) {
    ...
    pixelatedImage(ctx, enemyImg, enemyX, enemyY, enemySize, true);
  } else {
  ...
  // ── PLAYER sprite — anchored directly to the player battle circle ─────────
  const playerSize = 290;
  const playerCX = platforms.player.cx;
  const playerFootY = platforms.player.cy + 2;
  const playerX = playerCX - playerSize / 2;
  const playerY = playerFootY - playerSize;

  drawGroundShadow(ctx, playerCX, playerFootY + 8, 124);

  // The player-side trainer stands beside the player's Pokémon. Wild battles
  // intentionally leave the enemy side without a trainer.
  drawTrainerSprite(
    ctx,
    playerTrainerImg,
    Math.max(8, playerX - 96),
    playerFootY - 8,
    118,
    true,
  );
  ...
  if (enemyTrainerImg) {
    drawTrainerSprite(
      ctx,
      enemyTrainerImg,
      Math.min(W - 118, enemyX + enemySize - 18),
      enemyFootY - 6,
      104,
    );
  }
  ...
  if (playerImg) {
    // The back sprite naturally faces into the arena toward the enemy.
    pixelatedImage(ctx, playerImg, playerX, playerY, playerSize);
  } else {
  ...
  // ── HP boxes ──────────────────────────────────────────────────────────────
  // The message panel owns the upper-left like the reference battle screen.
  // Keep the enemy information in the upper-right, above its platform.
  drawArenaHPBox(ctx, W - 308, 12, 294, enemy.name, enemy.level, enemy.hp, enemy.maxHp, enemy.shiny, enemy.primaryType);
  ...
  drawDialogueBox(
    ...
    Math.min(580, W * 0.53),
    112,
    statusText || `${enemy.name} is ready to battle!`
  );
