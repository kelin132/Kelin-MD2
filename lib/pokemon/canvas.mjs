...
function getBattlePlatformLayout(W, H) {
  return {
    player: { cx: W * 0.28, cy: H * 0.74, rx: 152, ry: 30 },
    enemy: { cx: W * 0.72, cy: H * 0.40, rx: 128, ry: 26 },
  };
}

/** Draw the hard-edged battle circles and their rocky raised bases. */
function drawBattleCircle(ctx, { cx, cy, rx, ry }, accent, rock = "#4b4c4a") {
  ctx.save();
  ...
  ctx.beginPath();
  ctx.ellipse(cx, cy + 9, rx + 8, ry + 5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();

  const base = ctx.createRadialGradient(cx, cy - 5, 8, cx, cy, rx);
  base.addColorStop(0, "#77766b");
  base.addColorStop(0.58, rock);
  base.addColorStop(1, "#25292d");
  ctx.fillStyle = base;
  ctx.beginPath();
  ctx.ellipse(cx, cy, rx, ry, 0, 0, Math.PI * 2);
  ctx.fill();

  ctx.strokeStyle = "rgba(11,14,17,0.82)";
  ctx.lineWidth = 5;
  ctx.beginPath();
  ...
  ctx.globalAlpha = 0.8;
  ctx.beginPath();
  ctx.ellipse(cx, cy, rx - 11, ry - 8, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.globalAlpha = 0.34;
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.ellipse(cx, cy, rx - 28, ry - 14, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
}
...
function drawArenaBackground(ctx, W, H) {
  ...
  // Arena floor — a rocky, volcanic battle field inside a perspective boundary.
  ctx.save();
  ctx.beginPath();
  ...
  floor.addColorStop(0, "rgba(105,102,91,0.58)");
  floor.addColorStop(0.42, "rgba(62,65,64,0.72)");
  floor.addColorStop(1, "rgba(25,29,31,0.94)");
  ctx.fillStyle = floor;
  ctx.fillRect(0, 0, W, H);

  drawRockyTerrain(ctx, W, H);
  ...
  // Pokémon-style marked circles: their center lines are the sprite foot anchors.
  const platforms = getBattlePlatformLayout(W, H);
  drawBattleCircle(ctx, platforms.player, "rgba(136,205,255,0.9)", "#59635e");
  drawBattleCircle(ctx, platforms.enemy, "rgba(255,145,126,0.9)", "#655a55");
