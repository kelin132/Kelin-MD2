...
export async function generateBattleScene({ player, enemy, round = 1, hitSide = null, damage = null, crit = false, statusText = "" }) {
  ...
  if (enemyImg) {
    drawBattleSprite(ctx, enemyImg, enemyX, enemyY, enemySize, true);
  } else {
  ...
  // Opposing trainer stands on the upper-right patch and is mirrored so the
  // face/body direction points toward the player on the left.
  const enemyTrainerSize = 150;
  const enemyTrainerX = W * 0.79;
  const enemyTrainerY = enemyFootY - enemyTrainerSize + 8;
  drawGroundShadow(ctx, enemyTrainerX - enemyTrainerSize / 2, enemyFootY + 6, 48);
  drawTrainerSprite(
    ctx,
    enemyTrainerImg,
    enemyTrainerX - enemyTrainerSize / 2,
    enemyTrainerY,
    enemyTrainerSize,
    true
  );
  drawTrainerCaption(ctx, enemy.trainer?.name, enemyTrainerX, enemyTrainerY - 10);
  ...
  // Local trainer uses the back-facing FireRed sprite and stands to the
  // outside of their Pokémon, facing right toward the opponent.
  const playerTrainerSize = 150;
  const playerTrainerX = W * 0.07;
  const playerTrainerY = playerFootY - playerTrainerSize + 8;
  drawGroundShadow(ctx, playerTrainerX + playerTrainerSize / 2, playerFootY + 8, 54);
