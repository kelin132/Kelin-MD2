...
export async function generateBattleScene({ player, enemy, round = 1, hitSide = null, damage = null, crit = false, statusText = "" }) {
  ...
  // Opposing trainer stands beside their Pokémon and faces left, toward the
  // local side of the field, just like the opposing Pokémon sprite.
  const enemyTrainerSize = 132;
  const enemyTrainerX = W * 0.91;
  ...
  const playerTrainerSize = 132;
  const playerTrainerX = W * 0.018;
