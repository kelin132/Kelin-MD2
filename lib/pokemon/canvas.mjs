...
/**
 * Trainer sheets are 64px wide and some back sprites contain five 64px
 * animation frames stacked vertically. The first frame is the cleanest
 * standing pose for a still battle card.
 */
function drawTrainerSprite(ctx, image, x, y, size, flip = false) {
  if (!image) return;
  const sourceFrame = Math.min(image.width, image.height);
  ctx.save();
  if (flip) {
    ctx.translate(x + size, y);
    ctx.scale(-1, 1);
    ctx.drawImage(image, 0, 0, image.width, sourceFrame, 0, 0, size, size);
  } else {
    ctx.drawImage(image, 0, 0, image.width, sourceFrame, x, y, size, size);
  }
  ctx.restore();
}
...
export async function generateBattleScene({ player, enemy, round = 1, hitSide = null, damage = null, crit = false, statusText = "" }) {
  ...
  // Local trainer uses the complete back-facing FireRed sprite and stands
  // beside (not underneath) the player's Pokémon. The back sprite already
  // faces into the arena, so it must not be mirrored.
