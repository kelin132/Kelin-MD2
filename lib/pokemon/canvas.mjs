...
// ── Arena helpers (stadium aesthetic) ────────────────────────────────────────
...
function drawArenaBackground(ctx, W, H) {
  // Deep stadium gradient
  const bg = ctx.createLinearGradient(0, 0, W, H);
  bg.addColorStop(0,   "#10162a");
  bg.addColorStop(0.4, "#1e2d50");
  bg.addColorStop(0.55,"#26395e");
  bg.addColorStop(1,   "#111827");
  ctx.fillStyle = bg;
  ctx.fillRect(0, 0, W, H);

  // Arena floor with subtle perspective trapezoid
  ctx.fillStyle = "rgba(80,120,200,0.10)";
  ctx.beginPath();
  ctx.moveTo(W * 0.08, H * 0.48);
  ctx.lineTo(W * 0.92, H * 0.48);
  ctx.lineTo(W * 0.97, H - 10);
  ctx.lineTo(W * 0.03, H - 10);
  ctx.closePath();
  ctx.fill();
  ...
  const spot = ctx.createRadialGradient(W / 2, H * 0.38, 60, W / 2, H * 0.38, W * 0.55);
  ...
  ctx.beginPath();
  ctx.ellipse(W * 0.26, H * 0.72, 120, 24, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(100,160,255,0.15)";
  ctx.beginPath();
  ctx.ellipse(W * 0.26, H * 0.70, 120, 22, 0, 0, Math.PI * 2);
  ctx.fill();

  // Enemy platform (top-right)
  ctx.fillStyle = "rgba(0,0,0,0.2)";
  ctx.beginPath();
  ctx.ellipse(W * 0.74, H * 0.36, 100, 20, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,100,100,0.12)";
  ctx.beginPath();
  ctx.ellipse(W * 0.74, H * 0.34, 100, 18, 0, 0, Math.PI * 2);
  ctx.fill();
}

function drawArenaHPBox(ctx, x, y, w, name, level, hp, maxHp, shiny, primaryType) {
  // Panel
  roundRect(ctx, x, y, w, 88, 10);
  ctx.fillStyle = "rgba(255,255,255,0.93)";
  ctx.fill();
  const border = primaryType ? typeColor(primaryType) : "#2244aa";
  ctx.strokeStyle = border;
  ctx.lineWidth = 2.5;
  ctx.stroke();

  // Type color strip at top
  roundRect(ctx, x, y, w, 8, 10);
  ...
  ctx.textAlign = "left";
  ctx.fillText((name + (shiny ? " ✨" : "")).substring(0, 22), x + 10, y + 28);

  // Level
  ctx.font = "bold 13px Sans";
  ctx.fillStyle = "#444";
  ctx.textAlign = "right";
  ctx.fillText(`Lv.${level}`, x + w - 10, y + 28);

  // HP label
  ctx.fillStyle = "#333";
  ctx.font = "bold 12px Sans";
  ctx.textAlign = "left";
  ctx.fillText("HP", x + 10, y + 50);

  // HP bar background
  const barX = x + 30, barY = y + 40, barW = w - 46, barH = 11;
  roundRect(ctx, barX, barY, barW, barH, 5);
...
export async function generateBattleScene({ player, enemy, round = 1, hitSide = null, damage = null, crit = false, statusText = "" }) {
  ...
  roundRect(ctx, W / 2 - 72, 12, 144, 40, 20);
  ...
  ctx.font = "bold 18px Sans";
  ctx.textAlign = "center";
  ctx.fillText(`⚔  ROUND ${round}`, W / 2, 37);
  ...
  if (damage != null && hitSide) {
    ...
    if (crit) {
      ...
      ctx.fillStyle = "#ffd700";
      ctx.fillText("CRITICAL!", cx, cy - 38);
    }
    ctx.textBaseline = "alphabetic";
    ctx.restore();
  }
  ...
  if (statusText) {
    roundRect(ctx, 10, H - 56, W - 20, 44, 8);
    ...
    ctx.textAlign = "left";
    ctx.fillText(statusText.substring(0, 110), 22, H - 28);
  }
