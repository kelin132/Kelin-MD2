...
export async function addToParty(jid, pokemonId) {
  ...
  await col().updateOne({ jid }, { $push: { party: pokemonId } });
  return true;
}
...
/**
 * From an already-loaded party array, pick the trainer's designated lead.
 * Falls back to first alive Pokémon, then first in array.
 */
export function pickLeadFromParty(trainer, party) {
  const leadId = trainer?.leadPokemonId?.toString();
  if (leadId) {
    const lead = party.find(p => (p._id || p.id)?.toString() === leadId && (p.hp || 0) > 0);
    if (lead) return lead;
  }
  return party.find(p => (p.hp || 0) > 0) || party[0];
}
